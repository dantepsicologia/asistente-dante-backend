...
app.post('/webhook/mercadopago', async (req, res) => {
  ...
  try {
    ...
    // MP envía diferentes formatos según la versión
    const isPaymentEvent = type === 'payment' || action?.includes('payment');
    
    if (isPaymentEvent && data && data.id) {
      const paymentId = data.id;
      console.log(`🔍 Consultando pago ${paymentId} a MP...`);
      ...
      const payerEmail = paymentData.payer?.email || paymentData.payer?.payer?.email || 'no-email@example.com';
      const transactionAmount = paymentData.transaction_amount;
      const courseName = paymentData.additional_info?.items?.[0]?.title || 'Curso de Psicología';
      
      console.log(`💰 Pago ${paymentId} - Estado: ${status} - Email: ${payerEmail} - Monto: ${transactionAmount}`);
      
      if (status === 'approved') {
        const token = uuidv4();
        // Extraer duración del título del curso (ej: "Psicoestadística - 3 meses")
        const durationMatch = courseName.match(/(\d+)\s*meses?/i);
        const duration = durationMatch ? `${durationMatch[1]} meses` : '3 meses';
        ...
        console.log('📧 Enviando email a:', payerEmail);
        
        // Enviar email
        const emailSent = await sendActivationEmail(payerEmail, token, courseName, duration);
        ...
        console.log(`⏳ Pago ${paymentId} no aprobado aún. Estado: ${status}`);
      }
    } else {
      console.log('ℹ️ Webhook no es de tipo payment o no tiene data.id');
    }
    
    // SIEMPRE responder 200 a MP para que no reintegre
